...
(function (): void {
  ...
  interface BaseUrlMessage {
    type: "blk-spike2-base-url";
    baseUrl: string;
  }
  ...
  function isBaseUrlMessage(data: unknown): data is BaseUrlMessage {
    return (
      typeof data === "object" &&
      data !== null &&
      (data as { type?: unknown }).type === BASE_URL_TYPE &&
      typeof (data as { baseUrl?: unknown }).baseUrl === "string"
    );
  }

  function reportStep(step: string, ok: boolean, error?: string): void {
    reportedSteps.add(step);
    const message: StepMessage = { type: "blk-spike2-step", path: "A", step, ok };
    if (error !== undefined) message.error = error;
    window.postMessage(message, "*");
  }

  function reportRemainingAsFailed(error: string): void {
    for (const step of ALL_STEPS) {
      if (!reportedSteps.has(step)) reportStep(step, false, error);
    }
  }
  ...
  function describeMessageError(event: MessageEvent): string {
    return `messageerror origin="${event.origin}"`;
  }

  // -- Trusted Types blob worker construction ------------------------------

  async function buildTrustedWorkerUrl(baseUrl: string): Promise<TrustedScriptURL> {
    const factory = window.trustedTypes;
    if (!factory) throw new Error("window.trustedTypes is unavailable");
    ...
    if (!protocolResponse.ok) throw new Error(`protocol.js fetch failed: ${protocolResponse.status}`);
    const protocolSource = await protocolResponse.text();

    const separatorResponse = await fetch(`${baseUrl}assets/separator.js`);
    if (!separatorResponse.ok) throw new Error(`separator.js fetch failed: ${separatorResponse.status}`);
  ...
  function constructWorkerFromTrustedUrl(url: TrustedScriptURL): Worker {
    ...
    const WorkerCtor = Worker as unknown as new (scriptURL: TrustedScriptURL, options?: WorkerOptions) => Worker;
    return new WorkerCtor(url, { type: "module" });
  }

  // -- Worker round trip ----------------------------------------------------

  function runWorkerRoundTrip(trustedUrl: TrustedScriptURL, baseUrl: string): Promise<void> {
    return new Promise(resolve => {
      ...
      worker.addEventListener("message", event => {
        ...
        if (typeof data !== "object" || data === null || (data as { type?: unknown }).type !== "result") return;

        const result = data as {
          ortLoaded?: unknown;
          ortError?: unknown;
          webgpuSession?: unknown;
          webgpuError?: unknown;
        };
        if (result.ortLoaded === true) {
          ...
          reportStep(
            "ortWasmLoaded",
            false,
            typeof result.ortError === "string" ? result.ortError : "ORT did not load"
          );
        }
        if (result.webgpuSession === true) {
          ...
          reportStep(
            "webgpuSession",
            false,
            typeof result.webgpuError === "string" ? result.webgpuError : "webgpu session not built"
          );
        }
        finish();
      });
  ...
  async function runPathA(baseUrl: string): Promise<void> {
    try {
  ...
  window.addEventListener("message", event => {
    ...
    if (!isBaseUrlMessage(data)) return;
